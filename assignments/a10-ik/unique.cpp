...
public:
    ...
    void setup() {
        Motion motion;
        BVHReader reader;
        reader.load("../motions/Beta/walking.bvh", _skeleton, motion);
        motion.update(_skeleton, 0);

        _drawer.color = vec3(1, 0, 0);
        _head = _skeleton.getByName("Beta:Head");
        _lhand = _skeleton.getByName("Beta:LeftHand");
        _rhand = _skeleton.getByName("Beta:RightHand");
    }
    ...
    void lookAtTarget(Joint* head, const vec3& target) {
        ...
        head->setLocalTranslation(target);
        head->fk();
    }

    void lookAtTargethand(Joint* hand, const vec3& target) {
        // TODO: Your code here

        vec3 Z = target - hand->getGlobalTranslation();
        float length_of_z = sqrt((Z[0] * Z[0]) + (Z[1] * Z[1]) + (Z[2] * Z[2]));
        //vec3 Z2 = vec3(Z[0] / length_of_z, Z[1] / length_of_z, Z[2] / length_of_z);
        //vec3 Z2 = glm::compNormalize(Z);
        vec3 X = vec3(0, 1, 0) * Z;
    ...
    void scene() {
        ...
        lookAtTarget(_lhand, _target);
        lookAtTarget(_rhand, _target);
