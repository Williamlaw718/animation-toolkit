...
{
    vec3 desired;
    desired = actor.getPosition() - target;
    float length_of_d = sqrt((desired[0] * desired[0]) + (desired[1] * desired[1]) + (desired[2] * desired[2]));
    desired = desired / length_of_d;
    desired = desired * getParam("MaxSpeed");
    return desired;
}
